define("jquery-plugin/flickslide/1.0.3/flickslide-debug", [], function () {
	...
	return function ($) {
		$.fn.flickSlide = function (settings) {
			...
			function orientationChangeCore() {
				setTimeout(function () {
					var styles = getComputedStyle($(settings.parentArea, '.moveWrap').get(0));
					if (styles) {
						$(settings.parentArea, '.resizable').css('width', styles.width);
						$(settings.parentArea, '.slideMask').css('height', wrap.find('.move').outerHeight()).css('width', styles.width - 1);
						...
						$(settings.parentArea, 'div.flickSlideContainer ul.move').get(0).style.webkitTransform = 'translate3d(0,0,0)';
						currentX = 0;
						slideCount = 0;
						slidePager();
			...
			function slidePager() {
				...
				var currentPager = _bottom.find('.slidePagerPointer.active');
				var nextID = '#pager' + String(slideCount);
				currentPager.removeClass('active');
				_bottom.find(nextID).addClass('active');
				switch (slideCount) {
				case 0:
					_bottom.find('.bottomLeft').addClass('off');
					_bottom.find('.bottomRight').removeClass('off');
					break;

				case pagerMax:
					_bottom.find('.bottomRight').addClass('off');
					_bottom.find('.bottomLeft').removeClass('off');
					break;
